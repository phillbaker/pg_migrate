class PgMigratePostgresqlAT12 < Formula
  desc "PostgreSQL 12 extension and CLI to make schema changes with minimal locks"
  ...
  def install
    ENV.prepend "LDFLAGS", "-L#{Formula["postgresql@12"].opt_lib}"
    ENV.prepend "CPPFLAGS", "-I#{Formula["postgresql@12"].opt_include}"
    ENV.prepend "PKG_CONFIG_PATH", "-I#{Formula["postgresql@12"].opt_lib}/pkgconfig"
  ...
  def caveats
    <<~EOS
      ...
      cp #{lib}/pg_migrate.so #{Formula["postgresql@12"].lib}
      cp #{lib}/pg_migrate--#{version}.sql #{Formula["postgresql@12"].share}/postgresql@12/extension/
      cp #{lib}/pg_migrate.control #{Formula["postgresql@12"].share}/postgresql@12/extension/
