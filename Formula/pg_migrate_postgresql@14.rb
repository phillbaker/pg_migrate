class PgMigratePostgresqlAT14 < Formula
  desc "PostgreSQL 14 extension and CLI to make schema changes with minimal locks"
  ...
  def caveats
    <<~EOS
      ...
      cp #{lib}/pg_migrate.so #{Formula["postgresql@14"].lib}
      cp #{lib}/pg_migrate--#{version}.sql #{Formula["postgresql@14"].share}/postgresql@14/extension/
      cp #{lib}/pg_migrate.control #{Formula["postgresql@14"].share}/postgresql@14/extension/
