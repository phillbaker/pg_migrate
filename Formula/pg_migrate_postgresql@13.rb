class PgMigratePostgresqlAT13 < Formula
  desc "PostgreSQL 13 extension and CLI to make schema changes with minimal locks"
  ...
  def install
    ENV.prepend "LDFLAGS", "-L#{Formula["postgresql@13"].opt_lib}"
    ENV.prepend "CPPFLAGS", "-I#{Formula["postgresql@13"].opt_include}"
    ENV.prepend "PKG_CONFIG_PATH", "-I#{Formula["postgresql@13"].opt_lib}/pkgconfig"
  ...
  def caveats
    <<~EOS
      ...
      cp #{lib}/pg_migrate.so #{Formula["postgresql@13"].lib}
      cp #{lib}/pg_migrate--#{version}.sql #{Formula["postgresql@13"].share}/postgresql@13/extension/
      cp #{lib}/pg_migrate.control #{Formula["postgresql@13"].share}/postgresql@13/extension/
